...
function getDescriptionFromSlug(slug) {
  const allNavItems = [...navigationData.genres, ...navigationData.featuredCollections];
  const item = allNavItems.find(it => it.slug === slug);
  return item ? item.description : `Explore a wide variety of ${slug.replace(/-/g, ' ')} content. Download the latest movies in Mp4 format at Mp4Moviess.`;
}

export async function generateMetadata({ params }) {
  const param = await params;
  const { genreSlug } = param;
  const displayName = getDisplayNameFromSlug(genreSlug);
  // const displayDescription = 
   const displayDescription = getDescriptionFromSlug(genreSlug);
  
  return {
    title: `${displayName} | Download ${displayDescription}  | Latest Movies Download | mp4moviess.com`,
