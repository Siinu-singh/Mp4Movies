...
//   const trendingMovies = allMovies.filter(movie => movie.isTrending).sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 70);
//   const latestMovies = allMovies.filter(movie => movie.isNew).sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate)).slice(0, 70);
//   const upcomingMovies = allMovies.filter(movie => movie.isUpcoming).sort((a, b) => new Date(a.releaseDate) - new Date(b.releaseDate)).slice(0, 70);
...
//                   <div key={movie.id} className="w-[calc(30%-0.5rem)] md:w-[calc(33.33%-0.66rem)] lg:w-[calc(20.33%-0.66rem)] xl:w-[calc(25%-0.75rem)] flex-shrink-0">
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
        {allMovies.map((movie) => {
          ...
          const uniqueKey = `${movie.id || ''}-${movie.slug || ''}-${movie.title || ''}`.trim();
          return (
            <MovieCard 
              key={uniqueKey}
              movie={movie}
