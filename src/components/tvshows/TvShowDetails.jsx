...
import { Star, ChevronDown, ChevronUp, Play, Download, Loader2, RotateCcw, X, Heart } from 'lucide-react';
...
export default function TvShowDetails({ tvShow }) {
  ...
  if (!tvShow) {
    return <p className="text-center text-muted-foreground">TV Show details not found.</p>;
  }

  const pgRating = tvShow.pgRating || 'PG-13';
  const likes = tvShow.likes !== undefined ? tvShow.likes : Math.floor(Math.random() * 500) + 50;
  const releaseYear = tvShow.year || (tvShow.releaseDate ? new Date(tvShow.releaseDate).getFullYear() : 'N/A');
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={staggerContainer}>
      ...
      >
        <Image
          src={tvShow.backdrop_path || tvShow.poster || "https://placehold.co/1920x1080.png?text=Backdrop+Unavailable"}
          alt={`Backdrop for ${tvShow.title}`}
          fill
          sizes="100vw"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-10 lg:p-12 text-primary-foreground z-20">
          ...
          <motion.div variants={staggerItem} className="flex flex-wrap items-center gap-x-2 sm:gap-x-3 md:gap-x-4 gap-y-1 sm:gap-y-2 text-xs sm:text-sm md:text-base mb-4 md:mb-6">
            ...
            {tvShow.duration && <span className="text-foreground/70">{tvShow.duration}</span>}
          </motion.div>
          {/* Action Buttons */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 md:gap-8">
            ...
            <motion.div variants={staggerItem} className="md:col-span-4 lg:col-span-3">
              <div className="relative aspect-[2/3] w-full max-w-[200px] sm:max-w-xs mx-auto md:mx-0 rounded-md sm:rounded-lg shadow-xl overflow-hidden">
                <Image
                  src={tvShow.poster || "https://placehold.co/500x750.png?text=No+Poster"}
                  alt={tvShow.title}
                  fill
            ...
            <motion.div variants={staggerItem} className="md:col-span-8 lg:col-span-9">
              ...
              <div>
                ...
                <AnimatedReadMore text={tvShow.description} />
              </div>
              {tvShow.cast && tvShow.cast.length > 0 && (
                <div className="mt-3 sm:mt-4 md:mt-6">
                  ...
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    {tvShow.cast.slice(0, 7).map((actor) => (
                    ...
                    <div className="mt-8 text-center">
                      ...
                      >
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <span>Preparing your show experience...</span>
                        </div>
                      </motion.div>
                      <motion.div 
                ...
                {isDownloadLoading ? (
                  <div className="text-center">
                    ...
                    <motion.p 
                      ...
                      transition={{ delay: 0.5 }}
                    >
                      Preparing <span className="font-semibold text-foreground">{tvShow.title}</span> for download...
                    </motion.p>
                    <motion.div 
                  ...
                  <div className="text-center">
                    ...
                    >
                      ...
                      <p className="text-muted-foreground mb-4 sm:mb-6 text-xs sm:text-sm px-2">
                        Unable to generate download link for <span className="font-semibold text-foreground">{tvShow.title}</span>. Please try again.
                      </p>
                    </motion.div>
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
